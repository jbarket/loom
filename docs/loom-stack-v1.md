...
`~/.gemini/GEMINI.md`, `~/.opencode/AGENTS.md`) telling the agent to load
identity via loom at session start. Content outside the
`<!-- loom:start / loom:end -->` markers is preserved; re-running is idempotent.
...
| opencode       | `~/.opencode/AGENTS.md`       | `loom_`        |
